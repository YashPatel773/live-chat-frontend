...
const ChatWindow = () => {
  ...
  const { activeUser, isTyping, loading, messages } = useSelector((state) => state.chat);
  ...
  useEffect(() => {
    ...
    console.log("[ChatWindow] useEffect triggered for seen. activeUser:", activeUser.id, "messages length:", messages.length);
    dispatch(markAsSeen(activeUser.id));
    const socket = getSocket();
    if (socket) {
      console.log("[ChatWindow] Emitting messageSeen via socket.");
      socket.emit("messageSeen", { senderId: activeUser.id, receiverId: user.id });
    } else {
      console.warn("[ChatWindow] Socket not found when trying to emit messageSeen");
    }
  }, [activeUser, messages, user, dispatch]);

  const isActiveUserOnline = activeUser && onlineUserIds?.includes(String(activeUser.id));
      ...
      <div className="flex-1 h-screen bg-[#0a0a10] flex flex-col items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-violet-600/10 border border-violet-500/20 flex items-center justify-center">
            <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="rgba(139,92,246,0.7)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
            </svg>
          </div>
          <div className="text-center">
            <h3 className="text-[17px] font-bold text-slate-200 tracking-tight">No Conversation Open</h3>
    ...
    <div className="flex-1 h-screen flex flex-col bg-[#0a0a10]">
      ...
      <div className="relative flex items-center justify-between px-5 py-3.5 bg-[#0d0d1a]/95 border-b border-white/[0.06] backdrop-blur-md z-10">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-shrink-0">
            ...
            <span className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#0d0d1a]
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-500/[0.07] border border-emerald-500/15">
          ...
          <span className="text-[10px] font-semibold text-emerald-400/80 tracking-wide">Encrypted</span>
        </div>
      </div>
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-3">
          ...
          <span className="text-[12.5px] text-slate-500 tracking-wide">Loading messages…</span>
        </div>
      ) : (
        <MessageList />
      )}
