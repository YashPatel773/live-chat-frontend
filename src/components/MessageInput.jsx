...
const MessageInput = () => {
  ...
  const handleSend = (e) => {
    ...
    console.log(
      "[Socket] Attempting to send message. Socket instance exists:",
      !!socket,
    );
    ...
    dispatch(
      ...
      .then((savedMessage) => {
        ...
        if (socket) {
          ...
          console.log(
            "[Socket] 'sendMessage' event emitted to server with payload:",
            savedMessage,
          );
        } else {
          console.warn(
            "[Socket] Cannot emit: Socket is not connected/initialized!",
          );
        }
      })
      .catch((err) => {
